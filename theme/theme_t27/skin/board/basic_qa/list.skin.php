...
<div id="bo_list" style="width:<?php echo $width; ?>">
	...
	<div class="clearfix pr bl">
		...
		<div class="e_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px">
				...
				if (!$list[$i]['is_notice']){
					if(!$list[$i]['wr_1']){
						echo "<button type='button' class='btn btn-default btn-xs ko'><i class='fa fa-refresh' aria-hidden='true'></i>답변대기</button>&nbsp;";
					}else{
						echo "<button type='button' class='btn btn-danger btn-xs ko'><i class='fa fa-chevron-down' aria-hidden='true'></i>답변완료</button>&nbsp;";
					}
				}
				?>
				...
				if ($is_category && $list[$i]['ca_name']) {
				 ?>
				<a href="<?php echo $list[$i]['ca_name_href'] ?>" class="bo_cate_link ko f13"><strong>[<?php echo $list[$i]['ca_name'] ?>]</strong></a>&nbsp;
				<?php } ?>
		...
		<div class="m_view">
			<div class="e_writer"><i class="fa fa-user-o" aria-hidden="true"></i> 
				<?php

					// 이름 맨뒤에 OO
					//$list[$i]['name'] = substr($list[$i]['wr_name'],0,-6)."OO";
					//echo $list[$i]['name'];
