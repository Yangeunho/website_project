						...
						<?php } ?>
						  <section class="wrapper">
									...
									<?php for ($i=0; $i<count($list); $i++) {
									 ...
									 ?>
										<div class="col-xs-12 col-sm-6 col-xs-6 col-md-6 <?php echo $cols;?>" style='margin-bottom:30px;'>
											...
											$movies = $list[$i]['wr_10'];
											if($movies) {
												$basename = basename($movies);
												$url = str_replace("watch?v=","",$basename);
												$url = explode('&',$url);
											
											?>
											<div class="hover_container">
												<a href='<?php echo $list[$i]['href'] ?>'><img src="https://img.youtube.com/vi/<?=$url[0]?>/0.jpg" class="img_selector" width="100%"></a>

												<div class="overlay ko">
													<div class="hover_text">
														<div><?php echo $list[$i]['subject']?></div>
														<button type="button" class="info ko" data-toggle="modal" data-target="#modal1" ONCLICK="location.href='<?php echo $list[$i]['href'] ?>'">GO</button>
													</div>
												</div><!-- /card-content -->
											</div>
											<?php }else{ ?>
											...
											<!-------------------------- 
											해당 게시판은 이미지 + 유튜브를 동시에 사용하기 때문에
											간격을 별도로 조정할 필요가 있었습니다.
											style.css 하단쪽에 보시면
											
											pc, mobile 시에 이미지 세로길이나 텍스트 길이를 다르게
											제작했습니다.
											-------------------------->
											...
											<div class="hover_container">
												...
												<?php
													...
													if($thumb['src']) {
														$img_content = '<img src="'.$thumb['src'].'" alt="'.$thumb['alt'].'"  style="max-width:100%;">';
													} else {
												...
												<?php } else {
													...
													if($thumb['src']) {
														$img_content = '<img src="'.$thumb['src'].'" alt="'.$thumb['alt'].'" class="hover_images" style="max-width:100%;">';
													} else {
														$img_content = '<span class="no_image hover_images">no image</span>';
													}
												...
												<div class="overlay ko">
													<div class="hover_text">
														<div><?php echo $list[$i]['subject']?></div>
														<button type="button" class="info" data-toggle="modal" data-target="#modal1" ONCLICK="location.href='<?php echo $list[$i]['href'] ?>'">GO</button>
													</div>
												</div><!-- /card-content -->
											</div>
											<?php }//if?>
